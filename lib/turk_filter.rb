...
module TurkFilter
  ...
  def TurkFilter.get_filtered_trials(worker_id, stimset_id)
    ...
    @worker_filters.each do |filter|
      is_valid, score = filter.run_filter(trials)
      if not is_valid
  ...
  class WorkerFilter
    # Runs the filter and returns true if the worker created a valid
    # dataset and an optional score e.g. [is_true, score]
    def run_filter(trials)
      raise NotImplementedError
    end
  ...
  class TooRandom < WorkerFilter
    ...
    def run_filter(trials)
      score = calculate_p_random(trials)
  ...
  class TooManyClicksInSameSlot < WorkerFilter
    ...
    def run_filter(trials)
      ...
      trials.each do |trial|
        ...
        last_slot = cur_slot
        max_same_slot_count = [max_same_slot_count, same_slot_count].max()
      end

      is_valid = true
      if max_same_slot_count > @thresh
        is_valid = false
      end
      return [is_valid, max_same_slot_count]
    end
  end
